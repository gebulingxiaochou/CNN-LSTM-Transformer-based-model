...
def random_train(data_name, data_path, start_idx, num_features, save_path):
    ...
    search = product(config["num_layers"], config["hidden_size"])
    loss_rem1 = 0
    for idx, (i, j) in enumerate(search):
        para = {
            ...
            "hidden_size": j,
            "num_layers": i,
            "time_step": int(config["win_size"] - args.target_size),
        }
        if idx == 0:
        ...
        tCLN = TCLN(**para, device=device).to(device)
        epochs = config["random_epoch"]
        criterion = nn.MSELoss()
        optimizer = torch.optim.Adam(tCLN.parameters(), lr=config["learning_rate"])
        scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=config["g"])
        train_data = train_loader(int(para["time_step"] + args.target_size), config["batch_size"],
                                  data_path=data_path,
        ...
        for epoch in range(epochs):
            ...
            for idx, (data, label) in enumerate(tqdm(train_data)):
                ...
                output = tCLN(data)
                loss = criterion(output.squeeze_(), label.squeeze_())
...
def train_model(data_name, data_path, start_idx, num_features, save_path, para_path):
    ...
    for epoch in range(epochs):
        ...
        for idx, (data, label) in enumerate(tqdm(train_data)):
            ...
            if loss <= loss_rem2:
                loss_rem2 = loss
                torch.save(tCLN.state_dict(), save_path + f"{data_name}_train_param.pth")
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        scheduler.step()
