...
informer: Informer = Informer(**hyper_params, device=args.device)
informer.load_state_dict(torch.load(args.model_params_path))
...
with torch.no_grad():
    for length in predict_length:
        ...
        for i, (x, y) in enumerate(tqdm(test_data_loader)):
            ...
            pred_x: torch.Tensor = informer(data)[:, :length]
        ...
        plt.ylabel("value")
        plt.title(f"Informer&step{length}&{args.data_name}")
        plt.legend()
        plt.grid(alpha=0.3)
        plt.savefig(
            args.picture_path + f"/{length}_{args.data_name}_informer.png",
            dpi=300,
            bbox_inches="tight",
            pad_inches=1,
            facecolor="white"
