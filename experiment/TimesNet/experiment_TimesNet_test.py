...
timesnet: TimesNet = TimesNet(**hyper_params)
timesnet.load_state_dict(torch.load(args.model_params_path))
timesnet.to(args.device)
loss_mse: torch.nn.MSELoss = torch.nn.MSELoss(reduction="mean")
loss_mae: torch.nn.L1Loss = torch.nn.L1Loss(reduction="mean")
test_data_loader: DataLoader = train_loader(win_size=hyper_params["seq_len"] + 8,
                                            batch_size=32,
...
with torch.no_grad():
    for length in predict_length:
        ...
        for i, (x, y) in enumerate(tqdm(test_data_loader)):
            ...
            pred_x: torch.Tensor = timesnet(data)[:, :length]
